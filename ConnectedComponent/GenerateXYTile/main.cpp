...
string InputPath = "/scratch/jmmoon/connectedcomponent/";
string SavePath = "/scratch/jmmoon/connectedcomponent_tile/";
string DataType = "Format_Indexed8";

int whole_volume_x = 5120;
int whole_volume_y = 4608;
int whole_volume_z = 8000;
				...
				for (int lv = 0; lv <= sub_volume_level; ++lv) {
					...
					for (int y = 0; y < y_count; ++y) {
						for (int x = 0; x < x_count; ++x) {
							...
							//XY write step
							string tile_pathXY = SavePath + "/XY/" + to_string(z) + "_" + to_string(y) + "_" + to_string(x) + "_" + to_string(lv) + ".tile";
